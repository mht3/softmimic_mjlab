...
def base_lin_vel_penalty(
  ...
  """Penalize base XY linear velocity to encourage standing still."""
  asset: Entity = env.scene[asset_cfg.name]
  return torch.sum(torch.square(asset.data.root_link_lin_vel_b[:, :2]), dim=1)


def default_pose_penalty(
  env: ManagerBasedRlEnv,
  std: float,
  asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG,
) -> torch.Tensor:
  ...
  Returns mean squared error normalised by std^2, so the weight in RewardTermCfg
  controls the overall scale and std controls per-joint tolerance.
  """
  asset: Entity = env.scene[asset_cfg.name]
  error = asset.data.joint_pos[:, asset_cfg.joint_ids] - asset.data.default_joint_pos[:, asset_cfg.joint_ids]
  return torch.mean(torch.square(error) / std**2, dim=1)
...
def feet_in_contact(
  ...
  """Reward the fraction of feet that are in contact with the ground."""
...
def base_height_penalty(
  ...
  """Penalize deviation of the base height from a target standing height.

  Prevents the robot from crouching as a strategy to avoid falling.
  """
  asset: Entity = env.scene[asset_cfg.name]
